...
def corr_matrix_heatmap(df, save_fig = False, pp = None, title = None, color="RdYlGn"):  
    ...
    # I don't like seizmic, prefer greens are positive and reds as negative ... "PiYG
    im = ax.imshow(df, norm = plt.cm.colors.Normalize(-1,1), cmap = color)  
    ...
    for i in range(len(labels)):  
        for j in range(len(labels)):  
            text = ax.text(i, j, str(round(df.values[i][j],2)),  
                           ...
                           ha="center",   
                           va="center", 
                           color = heatmap_font_color(df.values[i][j]))
                           #color = "w")  
    #Create title with Times New Roman Font  
    #title_font = {"fontname":"Times New Roman"}  
    title_font = {"fontname":"Lucida Sans Unicode"}  
